...
Если **`https://serenity.agency/robots.txt`** выглядит как правила **WordPress**, поиск **`Disallow: /docs/`** ничего не находит — запрос почти наверняка уходит в **legacy** (старый контур на сервере), а не в файлы статики из репо. То же самое с **`/docs/team-handbook.html`**, если там **500**, страница **Nuxt** или другая ошибка legacy: нужны **актуальные файлы на диске** и **активированная маршрутизация/nginx** под префикс `/docs/` (см. [`CLAUDE.md`](CLAUDE.md), контекст маршрутизации).
