...
- **Без завершающего слэша** (sitemap, canonical, `href`, nginx 301 с `/…/`): `/blog`, `/blog/…`, `/case/all`, `/services`, `/targeting`, `/kontekstnaya_reklama` и пагинация/категории. **Исключение:** только главная **`https://serenity.agency/`**. Подстраницы **`/services/…`** (кроме листинга `/services`) — legacy.
...
- **Текст из задачи:** если автор передал тело статьи (целиком или по абзацам), **не дополнять** его от агента фразами, абзацами и ссылками «для связности»; ссылки вставлять только по **явному указанию** в задаче (URL или формулировка «слово X — ссылка на Y»). Допустимы правки разметки по регламенту и типографика.
- **Внутренние ссылки** — на услуги, кейсы, другие статьи, где уместно по смыслу; внешние — по необходимости, без битых URL.
