...
Правило Cursor для агента после **PROD** (`deploy-prod.sh`): [`.cursor/rules/post-prod-deploy-verification.mdc`](.cursor/rules/post-prod-deploy-verification.mdc) — базовый smoke + матрица тестов по затронутым зонам на `https://serenity.agency`.
