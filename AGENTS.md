...
| `**https://static.serenity.agency**`          | Dev-статика с диска `/var/www/static-dev` на том же сервере; для проверки превью — `**bash scripts/deploy-dev.sh**` (rsync в dev-root + purge CDN превью). Отдельно при необходимости — vhost превью: `**bash scripts/deploy-static-vhost.sh**`. Legacy Nuxt «Корпоративный сайт» для сравнения: `**https://static.serenity.agency/korporativnyj_sajt?legacy=1**` (алиас `**?v=legacy-preview**`; статика — `**/korporativnyj_sajt**` без этих query; CDN whitelist: `v`, `legacy`). |
