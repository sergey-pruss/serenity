#!/usr/bin/env node
/**
 * Скачивает /_nuxt/css/*.css со страницы /targeting, склеивает в css/korporativnyj-nuxt.bundle.css.
 */
const fs = require("fs");
...
const chunkDir = path.join(root, "tmp", "nuxt-korporativnyj-css-chunks");
const bundlePath = path.join(root, "css", "korporativnyj-nuxt.bundle.css");
const manifestPath = path.join(root, "korporativnyj_sajt", "nuxt-css-manifest.json");
const BUNDLE_HREF = "/_sa/css/korporativnyj-nuxt.bundle.css";
...
(async () => {
  if (!fs.existsSync(fullHtmlPath)) {
    console.error("Нет", fullHtmlPath, "— сначала: node scripts/capture-prod-korporativnyj-full-html.cjs");
    process.exit(1);
  }
  const origin = (process.env.KORPORATIVNYJ_NUXT_ORIGIN || process.env.TARGETING_NUXT_ORIGIN || "https://serenity.agency").replace(/\/$/, "");
