...
function link(href, text) {
  return `<a class="targeting-text-link" href="${href}">${text}</a>`;
}

/** Заголовок карточки — переход на страницу услуги 3-го уровня (видимая ссылка). */
function visibleServiceLink(href, text) {
  return `<a class="targeting-visible-text-link" href="${href}">${text}</a>`;
}
...
function buildAgencySection() {
  const items = [
    ...
    },
    {
      ...
      text: `Изучаем аудиторию, конкурентов и ${link("/uvelichenie-konversii-saita", "путь клиента до покупки")}. Сегментируем пользователей, тестируем офферы и креативы, анализируем результаты и перераспределяем бюджет в пользу наиболее эффективных связок.`,
    },
    {
  ...
  return buildSection("Агентство таргетированной рекламы", items, {
    description: `Агентство Serenity помогает бизнесу привлекать целевую аудиторию и получать заказы с помощью таргетированной рекламы. Строим ${link("/strategy", "стратегию")} на основе бизнес-целей, аналитики и экономики проекта, подбирая оптимальные площадки, форматы и аудитории — в том числе в рамках ${link("/services/marketing", "комплексного маркетинга")}.`,
    rowAligned: true,
  });
}
...
function buildSetupSection() {
  ...
  return buildSection("Настройка таргетированной рекламы", items, {
    description: `Настройка — фундамент эффективной рекламной кампании. Изучаем продукт и аудиторию, подбираем площадки, связываем таргет с ${link("/kontekstnaya_reklama", "контекстом")}, разрабатываем креативы и выстраиваем аналитику, чтобы после запуска получать заявки и продажи, а не расходовать бюджет на случайные показы.`,
    rowAligned: true,
  });
}
...
function buildPlatformsSection() {
  const dzenLink = visibleServiceLink(
    "/prodvizhenie-statey-v-dzene-i-promostranitsah",
    "Продвижение контента в Дзене",
  );
  const vkBlog =
    "/blog/article/pochemu-vashemu-biznesu-nuzhna-reklama-vo-vkontakte-i-pochemu-effektivnee-ee-delegirovat-professionalam";
  const smsBlog =
    "/blog/article/rabota-s-auditoriej-konkurentov-cherez-vk-reklamu-parsing-i-sms-kanaly";
  const telegramBlog = "/blog/article/kak-vybrat-kanaly-dlya-reklamy-v-telegram";
  const items = [
    {
      title: "Охватная реклама",
      text: `Показываем баннеры и видео широкой целевой аудитории ${link(vkBlog, "во VK, myTarget, SberAds")} и на других платформах. Используем социально-демографические характеристики, интересы, поведение и географию пользователей. Подходит для вывода нового продукта, роста узнаваемости бренда, продвижения акций и формирования спроса.`,
    },
    {
      title: "Таргетированные SMS",
      titleHtml: link(smsBlog, "Таргетированные SMS"),
...
function buildEffectivenessSection() {
  return buildSection(
    ...
    {
      rowAligned: true,
      description: `Оцениваем результат по системе KPI, согласованной с целями бизнеса: от охвата и привлечения подписчиков до заявок и прямых продаж. Сравниваем фактические показатели с медиапланом и корректируем стратегию на основе данных, сопоставляя результаты с ${link("/seo", "органическим трафиком")}.`,
    },
  );
}
  ...
  return buildSection(
    ...
    [
      ...
      },
      {
        ...
        titleHtml: link("/prodvizhenie-yandex-karty-2gis", "Локальное продвижение"),
        text: `Для бизнеса с физическими точками настраиваем рекламу по районам, станциям метро и выбранным геозонам. Используем данные операторов связи, цифровую наружную рекламу и ${link("/prodvizhenie-yandex-karty-2gis", "геотаргетинг")}, чтобы охватывать людей рядом с магазинами, офисами и мероприятиями.`,
      },
    ],
    {
      rowAligned: true,
