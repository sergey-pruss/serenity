...
function link(href, text) {
  return `<a class="kontekstnaya-text-link" href="${href}">${text}</a>`;
}
...
function buildAgencySection() {
  const items = [
    ...
    },
    {
      ...
      text: `Мы систематически анализируем все запущенные нами каналы рекламы в интернете, а также сайты конкурентов, после чего формируем предложения для офферов и отстройки. Знаем, как ${link("/blog/article/kak-privlechat-auditoriyu-konkurentov-cherez-yandeks-direkt/", "привлекать аудиторию конкурентов")} на сайты наших клиентов с соблюдением закона о рекламе.`,
    },
    {
...
function buildSetupSection() {
  const items = [
      ...
      {
        title: link("/uvelichenie-konversii-saita", "Проводим аудит посадочных страниц"),
      ...
      },
      {
        ...
        text: `Собираем ${link("/blog/article/seo-ili-kontekstnaya-reklama-chto-vybrat/", "семантическое ядро")}, формируем портреты аудиторий для показа рекламы. Распределяем бюджет между рекламными форматами и создаем медиаплан на период от 4 месяцев до 1 года. В медиаплане формируем прогноз трафика на сайт, количество и стоимость целевых действий. Подробный прогноз помогает определить, как будет работать контекстная реклама и какие гипотезы дадут наибольший эффект.`,
      },
      {
...
function buildManageSection() {
  const items = [
    ...
    },
    {
      ...
      text: `В работе делаем акцент не только на количестве заявок, но и на ${link("/uvelichenie-konversii-saita", "качестве трафика")}. Для нас результат — это рост целевых действий (звонков, заявок, ${link("/uvelichenie-konversii-saita", "продаж с сайта")}) при контролируемой стоимости обращения. Поэтому в процессе ведения мы синхронизируем рекламные показатели с бизнес-целями проекта, чтобы реклама приносила не просто клики, а понятный коммерческий эффект.`,
    },
  ];
...
function buildVidySection() {
  ...
  section = section.replace(
    ...
    `<h3 data-v-4ed7dc78="" class="block__name">${link("/prodvizhenie-yandex-karty-2gis", "Геомедийная контекстная реклама")}</h3>`,
  );
  section = section.replace(
    /<h3 data-v-4ed7dc78="" class="block__name"><a href="\/prodvizhenie-yandex-karty-2gis">Геомедийная контекстная реклама<\/a><\/h3>/g,
    `<h3 data-v-4ed7dc78="" class="block__name">${link("/prodvizhenie-yandex-karty-2gis", "Геомедийная контекстная реклама")}</h3>`,
  );
  section = section.replace(
    /в&nbsp;нишах с&nbsp;узкой аудиторией\./g,
    `в&nbsp;${link("/blog/article/kontekstnaya-reklama-dlya-b2b-v-2026-godu-kak-privlechat-lidy/", "нишах с&nbsp;узкой аудиторией")}.`,
  );
  section = section.replace(
    /Разместим рекламу в&nbsp;навигационных и&nbsp;картографических сервисах экосистемы Яндекса/g,
    `Разместим рекламу в&nbsp;${link("/blog/article/kontekstnaya-i-georeklama-premium-segment/", "навигационных и&nbsp;картографических сервисах")} экосистемы Яндекса`,
  );
  return section;
}
