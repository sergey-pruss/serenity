...
function buildCompareTable(data, options = {}) {
  const getRowIcon = options.getRowIcon || getKorporativnyjPackagesRowIcon;
  const mountId = options.mountId || "korporativnyj-packages-compare-mounted";
  ...
  const pinnedBodyRows = data.rows
    .map((row) => {
      const iconSvg = getRowIcon ? getRowIcon(row.label) : "";
      ...
      const thContent =
        row.text && !hasIcon
          ? labelBody
          : `<span class="kontekst-packages-compare__row-label-inner">${iconHtml}${labelBody}</span>`;
      return `            <tr${trClass}>
              <th scope="row" class="${labelClass}">${thContent}</th>
            </tr>`;
    })
    .join("\n");
...
<div id="${mountId}" class="kontekst-packages-compare">
...
function buildPackagesBlockHtml(rootDir, options = {}) {
  const {
    jsonRelPath = "json/services/korporativnyj_sajt/packages.json",
    sectionPrefix = "korporativnyj",
    compareMountId,
    markerTag = "KORPORATIVNYJ",
    getRowIcon = getKorporativnyjPackagesRowIcon,
  } = options;
  const mountId = compareMountId || `${sectionPrefix}-packages-compare-mounted`;
  const jsonPath = path.join(rootDir, jsonRelPath);
  const data = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
  const slides = data.cards
    .map((card, i) => buildPriceCardSlide(card, i + 1))
    .join("");
  const compare = buildCompareTable(data, { mountId, getRowIcon });

  return `<!-- ${markerTag}-PACKAGES-START -->
<section class="page-constructor__section ${sectionPrefix}-packages-heading"><div data-v-4ed7dc78="" class="modern content-block"><div data-v-4ed7dc78="" class="page__container"><div data-v-490c7534="" data-v-4ed7dc78="" class="numbered-header number-header__empty"><div data-v-490c7534="" class="row"><div data-v-490c7534="" class="col-6 col-md-12 numbered-header__title-column"><div data-v-490c7534="" class="numbered-header__bullet"></div> <div data-v-490c7534="" class="numbered-header__title"><h2 data-v-490c7534="">${data.heading}</h2> <!----> <h4 data-v-490c7534="" style="display: none;"></h4></div></div> <!----></div></div> <!----> <!----> <!----> <!----></div></div></section>
<section class="page-constructor__section ${sectionPrefix}-packages-dies"><div data-v-07db1e8a="" class="dies modern"><div data-v-07db1e8a="" class="page__container"><section data-v-1505791e="" data-v-07db1e8a="" class="prices"><div data-v-1505791e="" class="prices__cards row prices__cards--packages"><div data-v-1505791e="" class="prices__packages-slider swiper-container swiper-container-horizontal swiper-container-free-mode"><div data-v-1505791e="" class="prices__packages-track swiper-wrapper">${slides}</div></div></div>
${compare}
</section></div></div></section>
<!-- ${markerTag}-PACKAGES-END -->`;
}
