#!/usr/bin/env node
/**
 * Скачивает /_nuxt/css/*.css со страницы /targeting, склеивает в css/targeting-nuxt.bundle.css.
 */
const fs = require("fs");
...
const chunkDir = path.join(root, "tmp", "nuxt-targeting-css-chunks");
const bundlePath = path.join(root, "css", "targeting-nuxt.bundle.css");
const manifestPath = path.join(root, "targeting", "nuxt-css-manifest.json");
const BUNDLE_HREF = "/_sa/css/targeting-nuxt.bundle.css";
...
(async () => {
  if (!fs.existsSync(fullHtmlPath)) {
    console.error("Нет", fullHtmlPath, "— сначала: node scripts/capture-prod-targeting-full-html.cjs");
    process.exit(1);
  }
  const origin = (process.env.TARGETING_NUXT_ORIGIN || "https://serenity.agency").replace(/\/$/, "");
