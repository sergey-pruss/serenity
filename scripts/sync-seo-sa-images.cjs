#!/usr/bin/env node
/**
 * Скачивает в img/ отсутствующие storage__* из /_sa/img/ в seo/index.html (legacy /storage/ на prod).
 */
const fs = require("fs");
...
const htmlPath = path.join(root, "seo", "index.html");
...
(async () => {
  ...
  console.log(`sync-seo-sa-images: ${n} new file(s), ${names.size} referenced, ${fail} failed`);
