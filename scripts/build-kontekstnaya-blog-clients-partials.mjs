#!/usr/bin/env node
/**
 * Partial «Блог» и «Наши клиенты» для /kontekstnaya_reklama:
 * - blog: материалы, где контекстная реклама — тема поста (заголовок карточки или
 *   title/description статьи), плюс CURATED_EXTRA_SLUGS (контекст/Директ в теле);
 *   не раньше 2020 года; порядок — по publishDate, сначала новые (до 10 карточек)
 * - clients: копия html/partials/section-clients.html с главной
 */
import fs from "fs";
...
function wrapBlogPartial(blogHtml) {
  return `<section class="page-constructor__section kontekst-blog-section">
${blogHtml.trim()}
</section>`;
}
