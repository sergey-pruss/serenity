...
Dev-сервер запускает собранную статику (по умолчанию `http://127.0.0.1:8895/`). Порт: переменная `PORT` или скан до `8945`, если занят. В Safari строка без `:порт` — это **порт 80**, не `npm run dev`; копируйте URL из вывода терминала после старта.
